...
@State(name = "McpSettings_serp", storages = [Storage("McpSerpSettings.xml")])
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    fun getStdioConfig(): String {
        ...
        return """{"mcpServers": {"serp": {"command": "uvx", "args": ["mcp-serp"], "env": {"ACEDATACLOUD_API_TOKEN": "$$token"}}}}"""
    }

    fun getHttpConfig(): String {
        val token = myState.apiToken.ifEmpty { "YOUR_API_TOKEN" }
        return """{"mcpServers": {"serp": {"url": "https://serp.mcp.acedata.cloud/mcp", "headers": {"Authorization": "Bearer $$token"}}}}"""
    }
}
