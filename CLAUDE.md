...
MCP (Model Context Protocol) server for Google Search (SERP) web search via AceDataCloud API.
...
core/
  ...
  types.py      — Literal types (SearchType, etc.)
  exceptions.py — Error classes (AuthError, APIError, TimeoutError)
  utils.py      — Formatting helpers
tools/
  search_tools.py  — google search (web, images, news, etc.)
  info_tools.py    — list search types
...
1. **Source of truth** — `_docs/openapi/serp.json` is the OpenAPI spec for the SerpMCP API.
